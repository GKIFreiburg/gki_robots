...
void SensorProcessing::filterOdometry() 
{
  if(!_processOdometry)
    return;
  double mean_velocity = ((double) odometry_left_front_vel +
			  (double) odometry_right_front_vel +
			  double(odometry_left_rear_vel) + 
			  double(odometry_right_rear_vel)) / 4.0; 

  double mean_dist = ((double) odometry_left_front_dx + 
		      (double) odometry_right_front_dx +
		      double(odometry_left_rear_dx) +
		      double(odometry_right_rear_dx)) / 4.0; 

  odometry_vel_trans = (int)(((float)config->getWheelOutline() /
			      (float)WHEEL_TICKS_PER_CIRCULATION) *
			     ((float)mean_velocity));

  double new_dist = ((double)config->getWheelOutline() /
		     (double)WHEEL_TICKS_PER_CIRCULATION) *
    ((double)mean_dist);
...
void SensorProcessing::updateTime( timeval* time ) 
{
  ...
  if( starttime == 0 )
    {
      starttime = currenttime.tv_sec * 1000 +
	((currenttime.tv_usec / 1000) % 1000);
    }
}
...
long SensorProcessing::getRelativeTime_ms() 
{
  return (currenttime.tv_sec * 1000 + 
	  ((currenttime.tv_usec / 1000) % 1000)) - starttime;
}
...
//	"," << getOdometryPosTh() << ")" << std::endl;
      ...
      tValues.transVelLeftFront=(int)(odometry_left_front_vel*
				      ((float)config->getWheelOutline()/
				       (float)WHEEL_TICKS_PER_CIRCULATION));
      
      tValues.transVelLeftRear=(int)(odometry_left_rear_vel
				     *((float)config->getWheelOutline()/
				       (float)WHEEL_TICKS_PER_CIRCULATION));
      
      tValues.transVelRightFront = (int)(odometry_right_front_vel*
					 ((float)config->getWheelOutline() /
					  (float)WHEEL_TICKS_PER_CIRCULATION));
      
      tValues.transVelRightRear = (int)(odometry_right_rear_vel*
					((float)config->getWheelOutline()/
					 (float)WHEEL_TICKS_PER_CIRCULATION)); 
      // --CHANGES:END--
      
      if( config->enabledOdometrylf() || config->enabledOdometrylr() || 
	  config->enabledCompass2() || config->enabledOdometryrf() || 
	  config->enabledOdometryrr() || config->enabledOdometryrot()) 
	{
	  detectSlip(tValues);
      pubOdom.publish(msg);
	}
