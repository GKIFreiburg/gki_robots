import roslib;roslib.load_manifest('gki_turtlebot_dashboard')
import rospy
...
class KobukiDashboard(Dashboard):
    ...
    def get_widgets(self):
        ...
        return [[MonitorDashWidget(self.context), ConsoleDashWidget(self.context), self._motor_widget], leds, [self._laptop_bat, self._laptop_bat2, self._kobuki_bat]]
    ...
    def dashboard_callback(self, msg):
        ...
        if (laptop_battery_2_status):
            percentage = float(laptop_battery_2_status['Charge (Ah)'])/float(laptop_battery_2_status['Capacity (Ah)'])
            self._laptop_bat2.update_perc(percentage*100)
            self._laptop_bat2.update_time(percentage*100)
            charging_state = True if float(laptop_battery_2_status['Current (A)']) > 0.0 else False
