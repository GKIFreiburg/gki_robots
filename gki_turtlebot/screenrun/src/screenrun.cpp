...
class ProgramEntry
{
    public:
        ...
        void pushToScreen() {
            ...
            if(!executeCmd(g_screen_cmd + " -S " + session_name + " -X screen -t '" + name + "'"))
                return;
            for(vector<string>::iterator it = commands.begin(); it != commands.end(); it++) {
                ROS_INFO("Pushing command: \"%s\"", it->c_str());
                if(!executeCmd(g_screen_cmd + " -p '" + name + "' -S " + session_name + " -X eval 'stuff \"" + *it + "\"'"))
                    return;
            }
        }
};
...
bool screenRunning()
{
    ...
    while( (entry = readdir(dir)) ) {
        ...
        if(dname.find(session_name) != string::npos)
            return true;
    }
    return false;
}
...
int main(int argc, char** argv)
{
    ...
    ROS_INFO("Using session name: %s", session_name.c_str());
    ...
    if(screenRunning()) {
        ROS_WARN("Screen \"%s\" already running, reusing this session", session_name.c_str());
    } else {
        ROS_INFO("Creating screen \"%s\"", session_name.c_str());
        if(!executeCmd(g_screen_cmd + " -S " + session_name + " -d -m")) {
            ROS_FATAL("failed");
            return 1;
        }
    }
